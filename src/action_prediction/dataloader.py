...
def get_data_split(data_dir, split_file, candidate_results=None, is_train=False):
    print('get data split',split_file)
    def flatten_actions(samples):
        ...
        num_actions = [len(actions) for actions in samples.get("actions")]
    ...
    def parse_candidates(sample):
        ...
        # Add this - parse operation too
        if isinstance(sample["operation"], str):
            sample["operation"] = json.loads(sample["operation"])
        return sample
    dataset = dataset.map(parse_candidates)
    def add_previous_actions(sample):
        sample["previous_actions"] = sample.get("action_reprs", [])
        return sample

    dataset = dataset.map(add_previous_actions)

    flatten_dataset=dataset
    print(f'dataset is {flatten_dataset[0].keys()}')
            ...
            for candidates in [sample["pos_candidates"], sample["neg_candidates"]]:
                for candidate in candidates:
                    ...
                    if sample_id in candidate_scores and candidate_id in candidate_scores[sample_id]:
                        ...
                        candidate["score"] = 0.0
                        candidate["rank"] = 999  # push to bottom
            return {
